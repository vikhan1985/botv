...
def load_yaml(path: Path):
    ...
    if ylib_name == "ruamel":
        yaml = ylib.YAML(); yaml.preserve_quotes = True
    ...
    if actions.normalize_config and cfg.exists():
        ...
        for old, new in RENAME_KEYS.items():
            if old in mapping and new not in mapping:
                val = mapping.pop(old); mapping[new] = val
...
def main():
    ...
    ap.add_argument("--archive-bak", action="store_true", help="Переместить *.bak в archive/", default=False)
    ...
    actions = Actions(
        ...
        archive_bak=args.archive_bak if hasattr(args, "archive_bak") else False,
